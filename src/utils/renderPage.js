...
import getWebpackStats from "ff-utils/extras";
import config from "config";
import _ from "lodash";
import configureStore from "ff-server/configureStore";
import HtmlDocument from "ff-utils/html";
import Routes from "ff-shared/routes";
import translations from "../locale/index";
import Cookies from "cookies";
import { createCookieMiddleware } from "redux-cookie";
import { getData } from "./fetchData";
import getLanguage from "ff-utils/getLanguage";
...
export const renderAdmin = (req, res) => {
  ...
  const Pages = require(`ff-shared/admin/routes`).default;
  ...
  const store = configureStore(context, createCookieMiddleware(cookies));
